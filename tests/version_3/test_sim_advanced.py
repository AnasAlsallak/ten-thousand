...
# pytestmark = [pytest.mark.virsion]


def test_repeat_roller():
    """Allow setting aside scoring dice and rolling the rest
    """
    diffs = diff(play, path="tests/virsion_3/repeat_roller.sim.txt")
...
def test_hot_dice():
    ...
    """
    diffs = diff(play, path="tests/virsion_3/hot_dice.sim.txt")
    assert not diffs, diffs
...
def test_cheat_and_fix():
    ...
    diffs = diff(play, path="tests/virsion_3/cheat_and_fix.sim.txt")
    assert not diffs, diffs
...
def test_zilcher():
    ...
    diffs = diff(play, path="tests/virsion_3/zilcher.sim.txt")
    assert not diffs, diffs
